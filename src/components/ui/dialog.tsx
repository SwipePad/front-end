...
const ResponsiveModalVariants = cva(
  cn(
    "fixed z-50 gap-4 bg-card p-6 shadow-lg transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500 overflow-y-auto",
...
>(({ side = "bottom", className, children, closeable = true, ...props }, ref) => (
  <ResponsiveModalPortal>
    ...
    >
      {children}
      {closeable && (
        <ResponsiveModalClose className="ring-offset-background focus:ring-ring data-[state=open]:bg-secondary absolute right-6 top-6 rounded-sm opacity-70 transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:pointer-events-none">
          <X className="h-6 w-6" />
